...
const AdminTours = () => {
  ...
  const transportIcons = { Car: "🚗", Van: "🚐", Bus: "🚌", Boat: "⛴️", Train: "🚆" };
  ...
  const styles = {
    formStyle: { maxWidth: "95%", margin: "20px auto", padding: 20, background: "#f9f9f9", borderRadius: 10, boxShadow: "0 4px 8px rgba(0,0,0,0.1)" },
    inputStyle: { width: "100%", padding: 10, margin: "10px 0", borderRadius: 5, border: "1px solid #ccc", fontSize: "1rem", boxSizing: "border-box" },
    buttonStyle: { backgroundColor: "#2c5d30", color: "white", border: "none", padding: "10px 20px", borderRadius: 5, cursor: "pointer", marginRight: 10, marginTop: 10, width: "100%", maxWidth: 120 },
    cardStyle: { position: "relative", background: "#fff", padding: 15, borderRadius: 10, boxShadow: "0 2px 5px rgba(0,0,0,0.1)", border: "3px solid #2e7d32" },
  ...
  return (
    <div style={{ maxWidth: 1500, margin: 20, padding: 30, background: "linear-gradient(135deg, #c8f5d9, #4caf50)", borderRadius: 16 }}>
      ...
      <form ref={formRef} onSubmit={handleSubmit} style={styles.formStyle}>
        ...
        <select style={styles.inputStyle} value={destinationId} onChange={(e) => setDestinationId(e.target.value)} required>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 15, marginBottom: 15 }}>
          {transportOptions.map((option) => (
            <label key={option} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <input type="checkbox" checked={transport.includes(option)} onChange={(e) => {
        ...
        {transport.map((t) => (
          <div key={t} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
            <span style={{ minWidth: 80 }}>{transportIcons[t]} {t}</span>
            <input type="number" placeholder={`Price for ${t}`} style={styles.inputStyle} value={transportPrices[t] || ""} onChange={(e) => setTransportPrices({ ...transportPrices, [t]: e.target.value })} />
          </div>
        ))}
        ...
        {itinerary.map((item, index) => (
          <div key={index} style={{ marginBottom: 10 }}>
            ...
            <input style={styles.inputStyle} value={item.description} onChange={(e) => updateItineraryItem(index, e.target.value)} placeholder="Description" />
            <button type="button" onClick={() => removeItineraryItem(index)} style={{ ...styles.buttonStyle, backgroundColor: "#c0392b", maxWidth: 100 }}>Remove</button>
          </div>
        ))}
        <button type="button" onClick={addItineraryItem} style={{ ...styles.buttonStyle, maxWidth: 120 }}>+ Add Day</button>
        ...
        <div style={{ textAlign: "center" }}>
          <button type="submit" style={{ ...styles.buttonStyle, maxWidth: 250 }}>{editId ? "Update" : "Add"}</button>
          <button type="button" onClick={resetForm} style={{ ...styles.buttonStyle, backgroundColor: "#c0392b", maxWidth: 250 }}>Clear</button>
        </div>
      </form>
  ...
  {["all", "day", "round"].map((filter) => (
    <button
      ...
      style={{
        ...
        border: "none",
        cursor: "pointer",
        backgroundColor: mainFilter === filter ? "#2e7d32" : "#a5d6a7",
        color: mainFilter === filter ? "#fff" : "#2e7d32",
        fontWeight: 600,
      }}
    >
      {filter === "all" ? "All" : filter === "day" ? "Day Tour" : "Round Tour"}
    </button>
  ))}

  {/* Sub Filter */}
  {["all", "special", "regular"].map((filter) => (
    <button
      key={filter}
      ...
      style={{
        ...
        border: "none",
        cursor: "pointer",
        backgroundColor: subFilter === filter ? "#2e7d32" : "#a5d6a7",
        color: subFilter === filter ? "#fff" : "#2e7d32",
        fontWeight: 600,
      }}
    >
      {filter === "all" ? "All" : filter === "special" ? "Special" : "Regular"}
    </button>
  ))}
...
{/* Tours Grid */}
<div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(280px, 1fr))", gap: 20, marginTop: 30 }}>
  {filteredTours.map((tour) => (


          <div key={tour._id} style={styles.cardStyle}>
            {tour.isSpecial && <div style={{ position: "absolute", top: 10, left: 10, backgroundColor: "orange", color: "white", padding: "5px 10px", borderRadius: 5 }}>⭐ Special</div>}
            ...
            {tour.transport && tour.transport.length > 0 && (
              <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                {tour.transport.map((t, idx) => (
                  <li key={idx}>{transportIcons[t] || "❓"} {t} - LKR {Number(tour.transportPrices?.[t] || 0).toLocaleString()}</li>
                ))}
              </ul>
            )}

            <p style={styles.infoStyle}>⭐ {Array.from({ length: 5 }, (_, i) => <span key={i} style={{ color: i < tour.rating ? "#ffc107" : "#ddd", fontSize: "1.1rem" }}>★</span>)}</p>
            ...
            <div style={{ marginTop: 10 }}>
              <span
                ...
                style={{ display: "inline-flex", alignItems: "center", color: "#2e7d32", fontWeight: 600, cursor: "pointer" }}
              >
                {itineraryVisibility[tour._id] ? "Hide Itinerary" : "View Itinerary"} <span style={{ display: "inline-block", marginLeft: 6, transform: itineraryVisibility[tour._id] ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.3s ease" }}>▶</span>
              </span>
              ...
              <CSSTransition in={itineraryVisibility[tour._id]} timeout={300} classNames="slide" unmountOnExit>
                <div style={{ ...styles.itineraryBox, background: "#d4f4dd", padding: 15, marginTop: 10 }}>
                  {tour.itinerary && tour.itinerary.length > 0 ? (
                    <ol>
                      {tour.itinerary.map((item, idx) => <li key={idx}><strong>Day {item.day}:</strong> {item.description}</li>)}
                    </ol>
            ...
            <div style={{ textAlign: "center", marginTop: 10 }}>
              <button onClick={() => handleEdit(tour)} style={{ ...styles.buttonStyle, backgroundColor: "#81C784", width: 100 }}>Edit</button>
              <button onClick={() => handleDelete(tour._id)} style={{ ...styles.buttonStyle, backgroundColor: "#E57373", width: 100 }}>Delete</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {popup && (
        <div style={{ position: "fixed", top: 20, right: 20, backgroundColor: popupType === "success" ? "#4CAF50" : "#d32f2f", color: "#fff", padding: "12px 20px", borderRadius: 6, zIndex: 9999, fontWeight: "bold" }}>
          {popup}
        </div>
      )}
    </div>
