    ...
    >
      ...
      >
        ...
        <div className="relative z-10 px-6 md:px-20">
          <h1 className="text-2xl sm:text-3xl md:text-5xl text-green-950 font-semibold drop-shadow-md mb-4 sm:mb-6">
        ...
        <div className="flex items-center w-[80%] sm:w-[70%] max-w-md mx-auto mt-4 sm:mt-5 rounded-full overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 z-10">
          <div className="flex items-center flex-1 bg-white px-5 sm:px-6 h-12 sm:h-14">
            ...
            <input
              ...
              className="flex-1 text-sm sm:text-base text-gray-700 placeholder-gray-400 bg-transparent border-none focus:outline-none focus:ring-0"
            />
          </div>
      ...
      <div className="bg-green-50 w-full max-w-[1500px] p-6 sm:p-8 rounded-xl mt-10 shadow-lg">
        {filtered.length === 0 ? (
          ...
          >
            {filtered.map((exp) => {
              const imageUrl = `${axios.defaults.baseURL}${exp.imageUrl}`;
